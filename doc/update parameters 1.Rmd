...
lamda<-runif(dim(dtm_train_tfidf)[2],min = 5,max = 10)
lamda2<-runif(dim(dtm_train_tfidf)[2],min = 5,max = 10)
...
UpdateMatrix<-function(lamda,result_hclust1,result_hclust2){
  
  for (i in 1:length(unique(result_hclust1))){
       
       cluster<-unique(result_hclust1)
       num<-count[result_hclust1 == cluster[i]]
       
       if (length(num) > 1){
       f1[,i]<-colMeans(dtm_train_tfidf[num,])
       }
       else{
       f1[,i]<-dtm_train_tfidf[num,]
       }
  }
       f1<-colMeans(t(f1))
  for (j in 1:length(unique(result_hclust2))){
       
       cluster<-unique(result_hclust2)
       num<-count[result_hclust1 == cluster[i]]
       
       if (length(num) > 1){
       f2[,i]<-colMeans(dtm_train_tfidf[num,])
       }
       else{
       f2[,i]<-dtm_train_tfidf[num,]
       }
  }
       f2<-colMeans(t(f1))
